...
如果希望全面并且深入地学习 C++，读者需要查阅更多的参考资料，这里给出一些推荐的资源：

\begin{labeling}{\bfseries cppreference.com\cite{cppreference.com}}
  \item[\bfseries C++ Primer\cite{Lippman2013C}] 是 C++ 的经典教程，内容覆盖面广并且细节比较多。建议初学者在学习和使用 C++ 的过程中查阅而不是从头到尾地通读（否则你很可能记不住前面阅读过的大量知识）；
  \item[\bfseries cppreference.com\cite{cppreference.com}] 一个 C 和 C++ 语言和标准库的参考网站，对语言的各个版本各种细节都有详细地描述，适合有经验的读者使用（这个网站可以切换中文）。
\end{labeling}
